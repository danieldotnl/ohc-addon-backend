"""Test cases for HomeAssistantService."""
...
from ohc_backend.services.settings import HAConfig
...
class TestHomeAssistantService:
    ...
    @pytest.fixture
    def ha_service(self) -> HomeAssistantService:
        """Create a test instance of HomeAssistantService with patched session."""
        with patch("aiohttp.ClientSession"):
            service = HomeAssistantService()
            config = HAConfig(server="http://localhost:8123", token="test_token")  # noqa: S106
            service.configure(config)
        return service

    async def test_make_request_success(self, ha_service: HomeAssistantService) -> None:
        ...
        with patch.object(ha_service, "session") as mock_session:
            ...
            mock_session.request.assert_called_once_with("GET", "http://test/endpoint")

    async def test_stop(self, ha_service: HomeAssistantService) -> None:
        ...
        # Call the method
        await ha_service.stop()

        # Verify the result
        mock_session.close.assert_called_once()

    async def test_json_to_yaml(self, ha_service: HomeAssistantService) -> None:
    ...
    async def test_get_automation_content(self, ha_service: HomeAssistantService) -> None:
        ...
        with (
            patch.object(
                ha_service, "make_request", new_callable=AsyncMock, return_value=automation_dict
            ) as mock_request,
            patch.object(ha_service, "json_to_yaml", return_value="name: Test Automation\n") as mock_yaml,
        ):
            result = await ha_service.get_automation_content("test_auto")

            mock_request.assert_called_with("GET", "http://localhost:8123/api/config/automation/config/test_auto")
            mock_yaml.assert_called_with(automation_dict)
            assert result == "name: Test Automation\n"

    async def test_make_request_not_found(self, ha_service: HomeAssistantService) -> None:
    ...
    async def test_make_request_error(self, ha_service: HomeAssistantService) -> None:
    ...
    async def test_get_script_content(self, ha_service: HomeAssistantService) -> None:
        """Test getting script content."""
        script_dict = {"id": "test_script", "name": "Test Script"}

        with (
            patch.object(ha_service, "make_request", new_callable=AsyncMock, return_value=script_dict) as mock_request,
            patch.object(ha_service, "json_to_yaml", return_value="name: Test Script\n") as mock_yaml,
        ):
            result = await ha_service.get_script_content("script.test_script")

            mock_request.assert_called_with("GET", "http://localhost:8123/api/config/script/config/test_script")
            mock_yaml.assert_called_with(script_dict)
            assert result == "name: Test Script\n"

    async def test_get_automation(self, ha_service: HomeAssistantService) -> None:
        ...
        mock_state = {
            ...
            "state": "on",
            "attributes": {"id": "test_auto", "friendly_name": "Test Automation"},
            "last_changed": "2023-01-01T00:00:00Z",
        }

        with patch.object(ha_service, "make_request", new_callable=AsyncMock, return_value=mock_state):
    ...
    async def test_get_script(self, ha_service: HomeAssistantService) -> None:
    ...
    async def test_get_all_automations_and_scripts(self, ha_service: HomeAssistantService) -> None:
        ...
        with patch.object(ha_service, "make_request", new_callable=AsyncMock, return_value=mock_states):
