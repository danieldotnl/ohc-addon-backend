...
from ohc_backend.health import HealthState, HealthStatus, Service
...
def test_service_state_setting() -> None:
    ...
    hs.set_service_state(Service.GITHUB, HealthState.UNINITIALIZED)
    assert hs.get_state(service=Service.GITHUB) == HealthState.UNINITIALIZED
    assert hs.get_state() == HealthState.UNINITIALIZED

    # Update to AVAILABLE
    hs.set_service_state(Service.GITHUB, HealthState.AVAILABLE)
    assert hs.get_state(service=Service.GITHUB) == HealthState.AVAILABLE
    assert hs.get_state() == HealthState.AVAILABLE

    # Set another service to ERROR
    hs.set_service_state(
        Service.SYNCMANAGER, HealthState.ERROR, error_code=ErrorCode.INTERNAL_ERROR, description="Test error"
    )
    assert hs.get_state(service=Service.SYNCMANAGER) == HealthState.ERROR
    error_info = hs.get_service_error(Service.SYNCMANAGER)
...
def test_multiple_services_states() -> None:
    ...
    hs.set_service_state(Service.GITHUB, HealthState.AVAILABLE)
    hs.set_service_state(Service.SYNCMANAGER, HealthState.AVAILABLE)
    assert hs.get_state() == HealthState.AVAILABLE

    # When one service has UNINITIALIZED state, overall should be UNINITIALIZED
    hs.set_service_state(Service.HOMEASSISTANT, HealthState.UNINITIALIZED)
    assert hs.get_state() == HealthState.UNINITIALIZED

    # When one service has ERROR state, overall should be ERROR
    hs.set_service_state(Service.HOMEASSISTANT, HealthState.AVAILABLE)
    hs.set_service_state(
        Service.SYNCMANAGER, HealthState.ERROR, error_code=ErrorCode.GITHUB_API_ERROR, description="Another error"
    )
    assert hs.get_state() == HealthState.ERROR
...
def test_raise_on_error() -> None:
    ...
    hs.set_service_state(
        Service.SYNCMANAGER, HealthState.ERROR, error_code=ErrorCode.HOME_ASSISTANT_ERROR, description="Critical error"
    )
    ...
    assert error.message.startswith(f"Service '{Service.SYNCMANAGER}' in ERROR state")
    assert error.error_code == ErrorCode.HOME_ASSISTANT_ERROR
    assert "service" in error.details
    assert error.details["service"] == Service.SYNCMANAGER
...
def test_state_change_subscription() -> None:
    ...
    hs.set_service_state(
        Service.GITHUB, HealthState.ERROR, error_code=ErrorCode.SYNC_ERROR, description="Error for callback"
    )
...
def test_state_change_callback_not_called_on_same_state() -> None:
    ...
    hs.set_service_state(
        Service.HOMEASSISTANT, HealthState.ERROR, error_code=ErrorCode.VALIDATION_ERROR, description="Error state"
    )
...
def test_unsubscribe_from_state_changes() -> None:
    ...
    hs.set_service_state(
        Service.GITHUB, HealthState.ERROR, error_code=ErrorCode.AUTHENTICATION_FAILED, description="After unsubscribe"
    )
...
def test_service_removal() -> None:
    ...
    # Remove a service
    hs.remove_service(Service.GITHUB)
    assert Service.GITHUB not in hs.get_services()
    assert hs.get_state() == HealthState.AVAILABLE

    # Add a service in error state
    hs.set_service_state(
        Service.HOMEASSISTANT, HealthState.ERROR, error_code=ErrorCode.NOT_FOUND, description="Error state"
    )
    assert hs.get_state() == HealthState.ERROR

    # Remove the error service
    hs.remove_service(Service.HOMEASSISTANT)
    assert hs.get_state() == HealthState.AVAILABLE
...
def test_clear_error() -> None:
    ...
    hs.set_service_state(
        Service.GITHUB, HealthState.ERROR, error_code=ErrorCode.INTERNAL_ERROR, description="Test error"
    )
    assert hs.get_state(service=Service.GITHUB) == HealthState.ERROR
    ...
    with pytest.raises(KeyError):
        hs.get_service_error(Service.GITHUB)
